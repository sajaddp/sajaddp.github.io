# سجاد ده شیری

**Sajad Dehshiri (sajaddp)**

**سجاد ده شیری sajaddp | توسعه محصول + سئو فنی + منتورینگ + آموزش مذاکره تجاری (صفحه رسمی + لینک‌های رسمی)**

اگر این نام یا نام کاربری را جستجو کرده‌ای، این صفحه رسمی من روی sajaddehshiri.ir برای تایید هویت و دسترسی به لینک‌هاست.

این روزها بخشی از فعالیتم روی آموزش مذاکره تجاری متمرکز است — اطلاع‌رسانی از طریق کانال‌های زیر انجام می‌شود.

## کانال‌های اطلاع‌رسانی و جامعه

- Telegram: <https://t.me/sajad_dp>
- Rubika: <https://rubika.ir/sajaddp71>
- Eitaa: <https://eitaa.com/roadmap>
- Bale: <https://ble.ir/sajaddehshiri>
- Instagram: <https://www.instagram.com/sajaddp/>
- LinkedIn: <https://www.linkedin.com/in/sajaddp>

## محتوای ویدئویی

- YouTube: <https://www.youtube.com/@sajaddehshiri>
- Aparat: <https://www.aparat.com/roadmap1>

## حضور حرفه‌ای

- GitHub: <https://github.com/sajaddp>

> فقط همین کانال‌ها و پروفایل‌ها رسمی‌اند. آدرس‌های مشابه را به نام من حساب نکن.
>
> اطلاع‌رسانی مربوط به محتوا و دوره‌های آموزشی (از جمله مذاکره تجاری) فقط از طریق همین کانال‌ها و پروفایل‌های رسمی انجام می‌شود.

## چه کاری انجام می‌دهم

- **توسعه محصول:** از ایده تا لانچ، محصولی سریع، پایدار و قابل رشد می‌سازم.
- **سئو فنی:** ساختار و سرعت سایت را بهتر می‌کنم تا رتبه و نرخ تبدیل رشد کند.
- **منتورینگ:** جلسات عملی برای تصمیم‌های فنی، مسیر شغلی و رشد تیمی.
- **آموزش مذاکره تجاری:** این روزها بخشی از فعالیتم به آموزش کاربردی مذاکره در فضای کسب‌وکار اختصاص دارد.

## معرفی کوتاه

من سجاد ده شیری هستم. کار اصلی من ساخت و توسعه محصول‌های کاربردی با کیفیت فنی بالاست. در بک‌اند بیشتر با Laravel و Node.js و در فرانت‌اند با React و Next.js کار می‌کنم، با تاکید جدی بر عملکرد و سئو فنی.

در وب بیشتر با نام کاربری **sajaddp** شناخته می‌شوم و بخشی از زمانم را صرف کارهای متن‌باز، ابزارهای کاربردی و منتورینگ می‌کنم.

در این دوره زمانی، علاوه بر مسیر فنی، روی **آموزش مذاکره تجاری** هم تمرکز دارم و محتوای آموزشی و دوره‌های مرتبط را از طریق کانال‌های رسمی منتشر و اطلاع‌رسانی می‌کنم.

## About (English)

I’m **Sajad Dehshiri (sajaddp)**. I build maintainable products that ship fast, with a strong focus on performance and technical SEO.

Alongside product development and technical mentoring, these days I also focus on **business negotiation education** and share related learning content and course updates through my official channels.

This is my official page on **sajaddehshiri.ir** for identity verification and main links.