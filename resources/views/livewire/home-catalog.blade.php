<section class="flex flex-col gap-10">
    <div class="flex flex-col gap-2">
        <h2 class="text-2xl font-semibold text-slate-950">Latest courses</h2>
        <p class="text-sm text-slate-600">Browse the newest content and jump straight into YouTube.</p>
    </div>
    ...
    <div class="grid gap-6">
            ...
            <article class="rounded-3xl border border-slate-900/10 bg-white/70 p-6 shadow-sm backdrop-blur" wire:key="course-{{ $course['id'] }}">
                ...
                <div class="grid gap-4 pt-6 sm:grid-cols-2 xl:grid-cols-3">
                    @foreach ($course['videos'] as $video)
                        <a class="group flex h-full flex-col overflow-hidden rounded-2xl border border-slate-900/10 bg-white transition hover:-translate-y-1 hover:border-slate-900/20 hover:shadow-lg" href="{{ $video['youtube_url'] }}" target="_blank" rel="noopener" wire:key="video-{{ $video['id'] }}">
                            <div class="relative aspect-video w-full overflow-hidden bg-slate-100">
                                ...
                                @else
                                    <div class="flex h-full items-center justify-center text-sm font-medium text-slate-400">Thumbnail coming soon</div>
                                @endif
                            </div>
                            <div class="flex flex-1 flex-col gap-2 p-4">
                                <p class="text-base font-semibold text-slate-900">{{ $video['title'] }}</p>
                                <span class="text-xs font-medium uppercase tracking-[0.2em] text-amber-700">Watch on YouTube</span>
                            </div>
                        </a>
                    @endforeach
                </div>
        ...
        @empty
            <div class="rounded-3xl border border-dashed border-slate-300 bg-white/70 p-10 text-center text-sm text-slate-500">
                No courses yet. Add the first course and videos from the admin panel.
            </div>
        @endforelse
    </div>
</section>
